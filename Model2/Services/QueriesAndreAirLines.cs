...
namespace ModelShare.Services
{
    public class QueriesAndreAirLines
    {
        ...
        public static async Task<AirPlane> SearchAirplane(string Iata)
        {
            try
            {
                HttpResponseMessage response = await client.GetAsync("https://localhost:44331/api/AirPlane/" + Iata);
                response.EnsureSuccessStatusCode();
                string responseBody = await response.Content.ReadAsStringAsync();
        ...
        public static async Task<Class> SearchClass(string id)
        {
            try
            {
                HttpResponseMessage response = await client.GetAsync("https://localhost:44344/api/Class/" + id);
                response.EnsureSuccessStatusCode();
                string responseBody = await response.Content.ReadAsStringAsync();
